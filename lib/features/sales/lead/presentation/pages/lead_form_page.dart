...
import '../providers/lead_form_provider.dart';
...
class _LeadFormPageState extends State<LeadFormPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      context.read<LeadFormProvider>().initialize(
  ...
  void _syncControllers(List<LeadField> fields) {
  ...
  Future<void> _submit(BuildContext context) async {
    ...
    if (!success) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(provider.error ?? 'Could not save lead')),
      );
      return;
    }
  ...
  Widget build(BuildContext context) {
    final provider = context.watch<LeadFormProvider>();
    final fields = provider.fields;
    _syncControllers(fields);

    return Scaffold(
      appBar: AppBar(
        title: Text(widget.leadName == null ? 'Create Lead' : 'Edit Lead'),
        actions: [
          IconButton(
          ...
          children: [
            ...
            ...fields.map(
              (field) => Padding(
                ...
                child: _LeadFieldInput(
                  field: field,
                  controller: _controllers[field.key]!,
                  onChanged: (value) {
                    AppLogger.sales('lead form changed ${field.key}');
            ...
            FilledButton.icon(
              ...
              label: Text(widget.leadName == null ? 'Create Lead' : 'Update Lead'),
            ),
          ],
        ),
      ),
...
class _LeadFieldInput extends StatelessWidget {
  const _LeadFieldInput({
    required this.field,
    required this.controller,
    required this.onChanged,
  });

  final LeadField field;
  ...
  Widget build(BuildContext context) {
    ...
    final keyboardType = switch (field.type) {
      LeadFieldType.email => TextInputType.emailAddress,
      LeadFieldType.phone => TextInputType.phone,
      LeadFieldType.number => TextInputType.number,
      LeadFieldType.multiline => TextInputType.multiline,
      LeadFieldType.date => TextInputType.datetime,
    ...
    return TextFormField(
      ...
      maxLines: field.type == LeadFieldType.multiline ? 4 : 1,
      decoration: InputDecoration(
        labelText: field.required ? '${field.label} *' : field.label,
        alignLabelWithHint: field.type == LeadFieldType.multiline,
        filled: true,
...
class _SelectableField extends StatelessWidget {
  ...
  Future<void> _openPicker(BuildContext context) async {
    final selected = await showModalBottomSheet<LeadOptionItem>(
      context: context,
  ...
  @override
  Widget build(BuildContext context) {
    final helper = field.type == LeadFieldType.link
              ...
              child: Column(
                children: [
                  ...
                  TextField(
                    ...
                    autofocus: widget.field.type == LeadFieldType.link,
                    decoration: InputDecoration(
                      hintText: widget.field.type == LeadFieldType.link
