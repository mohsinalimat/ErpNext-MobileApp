...
class OpportunitiesProvider extends ChangeNotifier {
  ...
  List<Opportunity> get opportunities =>
      _applyLocalSort(_applyLocalFilter(_opportunities));
  ...
  List<Opportunity> _applyLocalSort(List<Opportunity> input) {
    final today = _todayOnly();
    final weekStart = today.subtract(Duration(days: today.weekday - 1));
    final weekEnd = weekStart.add(const Duration(days: 6));
    final items = [...input];

    int compareNextFollowUp(Opportunity a, Opportunity b) {
    ...
    int compareLastModifiedDesc(Opportunity a, Opportunity b) {
    ...
    items.sort((a, b) {
      final aState = _stateForOpportunity(
        a,
        today: today,
        weekStart: weekStart,
        weekEnd: weekEnd,
      );
      final bState = _stateForOpportunity(
        b,
        today: today,
        weekStart: weekStart,
        weekEnd: weekEnd,
