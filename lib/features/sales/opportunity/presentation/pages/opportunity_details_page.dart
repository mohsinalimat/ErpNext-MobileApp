...
class _OpportunitySummaryCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final company = data['customer_name']?.toString() ??
...
class _OpportunityWorkflowCard extends StatefulWidget {
  const _OpportunityWorkflowCard({
  ...
  @override
  State<_OpportunityWorkflowCard> createState() => _OpportunityWorkflowCardState();
}

class _OpportunityWorkflowCardState extends State<_OpportunityWorkflowCard> {
  bool _isSubmitting = false;
  ...
  Widget build(BuildContext context) {
    ...
    final workflowState = workflow.actions.isNotEmpty
        ? workflow.actions.first.state
        : '';
    final detailsWorkflowState = widget.data['workflow_state']?.toString().trim() ?? '';
    final currentStatus = workflowState.isNotEmpty
        ? workflowState
        : (detailsWorkflowState.isNotEmpty
            ? detailsWorkflowState
            : (widget.data['status']?.toString().trim().isNotEmpty == true
            ? widget.data['status'].toString().trim()
            : 'Open'));
    final salesStage = widget.data['sales_stage']?.toString() ?? '';
          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                ...
                if (workflow.workflowName.isNotEmpty)
                  _WorkflowInfoChip(label: workflow.workflowName),
                if (salesStage.isNotEmpty)
                  _WorkflowInfoChip(label: 'Stage: $salesStage'),
              ],
            ),
            const SizedBox(height: 10),
            Text(
              'Current Workflow State: $currentStatus',
              style: const TextStyle(
  ...
  Future<void> _applyWorkflowAction(BuildContext context, String action) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Opportunity moved to $action')),
      );
    } catch (_) {
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(provider.error ?? 'Failed to update workflow status'),
        ),
      );
    } finally {
      if (mounted) {
...
String _labelFromKey(String key) {
  const customLabels = {
    ...
    'opportunity_from': 'Opportunity From',
